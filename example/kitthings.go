...
func addMethod(at rs.ASTTemplate, root *ast.File, ifc rs.Interface, meth rs.Procedure) {
	s := ifc.Implementor()
	def := meth.MethodDefinition(s, at, "ExampleEndpoint")
	root.Decls = append(root.Decls, def)
}

func addRequestStruct(root *ast.File, meth rs.Procedure) {
	root.Decls = append(root.Decls, requestStruct(meth))
}

func addResponseStruct(root *ast.File, meth rs.Procedure) {
	root.Decls = append(root.Decls, responseStruct(meth))
}

func addEndpointMaker(at rs.ASTTemplate, root *ast.File, ifc rs.Interface, meth rs.Procedure) {
	s := ifc.Implementor()
...
func addDecoder(at rs.ASTTemplate, root *ast.File, meth rs.Procedure) {
	root.Decls = append(root.Decls, decoderFunc(at, meth))
}

func addEncoder(at rs.ASTTemplate, root *ast.File, meth rs.Procedure) {
